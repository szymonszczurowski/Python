...
print('min and max from percent:', min(percent), max(percent))

for i in range(0, len(countries)-1):
    print('percent value:', percent[i], ':', 'percent int:', int(percent[i]), 'percent round^:', round(percent[i], 2), 'points for the country:', countries[i], ':', int(round(percent[i]*sumOfPoints/100,0)))